...
\begin{izrek}[Ramanujanova domneva]\label{ramanujan-domneva}
    ...
    \end{align*}
\end{izrek}
Izrek je pravzaprav neposredna posledica Ramanujanove domneve, kot opisano, vendar ga bomo imenovali kar Ramanujanova domneva. Podobno kot pri dokazu Jacobijevega izreka o štirih kvadratih, ga ne bomo dokazali. Osredotočimo se raje na to, kako ta izraz uporabimo, da omejimo lastne vrednosti grafa. 
...
\begin{align*}
    s(p^m) = \frac{2}{n} p^{m}{2}\sum_{j=0}^{n-1}\frac{\sin((m+1)\theta_j)}{\sin \theta_j}
\end{align*}
za \(\lambda_j = 2\sqrt{p}\cos\theta_j\). Če so \(\theta_j\) vsi realni (z izjemo trivialnih vrednosti \(i\log\sqrt p\), ki nam da lastno vrednost \(p+1\) (A je to res??) in \(\pi + i \log\sqrt{p}\), ki nam da \(-(p+1)\)), potem bo \(\lambda_k \leq 2\sqrt{p}\), kar je pogoj za Ramanujanove grafe.
