...
\begin{izrek}
    ...
    \end{align*}
\end{izrek}
Podobno kot pri dokazu Jacobijevega izreka o štirih kvadratih, tega ne bomo dokazali. Osredotočimo se raje na to, kako ta izraz uporabimo, da omejimo lastne vrednosti grafa. 
...
\begin{dokaz}
    ...
    Te matrike imajo nekaj enostavnih rekurzivnih lastnosti. Prvi dve sta \(A_1^2 = A_2 + k\cdot I\) in \(A_1 A_l = A_lA_1 = A_{l+1}+(k-1)A_{l-1}\). Preko njih lahko enostavno dobimo tudi generatorsko funkcijo
    \begin{align*}
        \sum_{l=0}^\infty A_lt^l = \frac{1-t^2}{1-At+(k-1)t^2}
    \end{align*}
    ...
    \end{align*}
    Enostavno lahko poiščemo lastne vrednosti polinoma matrike. Namreč, če je \(p\) polinom in \(A\) matrika z lastno vrednostjo \(\lambda\), potem je \(p(\lambda)\) lastna vrednost matrike \(p(A)\). Torej lahko, če so \(\lambda_i\) lastne vrednosti \(A\), sled matrike \(T_m\) napišemo kot
    \begin{align*}
        \mathrm{Tr} T_m = (k-1)^{\frac{m}{2}}\sum_{j=1}^n U_m \left(\frac{\lambda_i}{2\sqrt{k-1}}\right).
    \end{align*}
    ...
    \begin{align*}
        \sum_{0\leq l\leq \frac{m}{2}} \rho(m-2l) = (k-1)^{\frac{m}{2}}\sum_{j=1}^n U_m \left(\frac{\lambda_i}{2\sqrt{k-1}}\right)
    \end{align*}
\end{dokaz}

Sedaj lahko združimo oba izreka skupaj, da dobimo enakost, ki povezuje število rešitev enačbe z lastnimi vrednostmi grafa
\begin{align*}
    s(p^m) = (k-1)^{\frac{m}{2}}\sum_{j=1}^n U_m \left(\frac{\lambda_i}{2\sqrt{k-1}}\right)
\end{align*}
