...
\begin{lema}[Sprehodi v drevesu]\label{alon:drevesa-meja}
    ...
    \begin{align*}
        \frac{1}{l}\binom{2l-2}{l-1}k(k-1)^{l-1} = \rho'(2l)
    \end{align*}
...
\begin{dokaz}
    ...
    \begin{align*}
        \frac{1}{l}\binom{2l-2}{l-1} d (d-1)^{l-1}.
    \end{align*}
\end{dokaz}
\begin{dokaz}[Dokaz Alon-Boppanovega izreka]
    Naj bo \(A_n\) sosednostna matrika \(G_n\). Potem je \((i,j)\)-ti element matrike \(A_n^k\) število poti od \(i\) do \(j\) dolžine \(k\), označimo ga \(\delta_{(i,j)}^k\). Če so lastne vrednosti matrike \(A_n\) enake \(d=\lambda_1\geq \lambda_2 \geq \cdots \geq \lambda_n\), potem so lastne vrednosti \(A_n^k\) enake \(\lambda_j^k\) za \(1\leq j \leq n\). Sled matrike je neodvisna od baze torej, če matriko napišemo v lastni bazi vidimo
    \begin{align*}
        \sum{j=1}^n \lambda_j^k = \sum_{j=1}^n \delta_{(j, j)}^k
    \end{align*}
\end{dokaz}
