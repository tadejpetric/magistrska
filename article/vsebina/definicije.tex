...
\end{izrek}
\begin{dokaz}
    Dokažimo najprej \(2c(G) \geq s(G)\). Naj bo \(S\subset V(G)\) tak, da zadosti minimumu iz definicije Cheegerjeve konstante, torej \(0<\lvert S\rvert \leq \frac12\lvert V(G)\rvert\) in 
    \begin{align*}
        c(G) = \frac{\lvert\partial S\rvert}{\lvert S \rvert}.
    \end{align*}
    ...
    Izberemo vektor \(v\) tako, da ima elemente enake \(\frac{1}{\lvert S\rvert}\) na indeksih, ki pripadajo elementom iz \(S\) ter ostale elemente (tiste, ki pripadajo indeksom za \(\overline S\)) enake \(-\frac{1}{\lvert \overline S \rvert}\). Ta vektor je pravokoten na vektor samih enic.
    Sedaj izračunamo Rayleighov kvocient.
    \begin{align*}
        Av = \left(\sum_{j=1}^{\lvert V(G)\rvert} v_j \cdot A_{i,j}\right)_{i=1}^{\lvert V(G)\rvert}\\
        v^\top Av = \sum_{i=1}^{\lvert V(G)\rvert} \sum_{j=1}^{\lvert V(G)\rvert} v_i v_j A_{i,j} = \sum_{i\sim j} v_i v_j\\
    \end{align*}
    \begin{align*}
        R(A, v) &= \frac{\sum_{i\sim j} v_i v_j}{\sum_{i=1}^{\lvert V(G)\rvert}v_i^2}\\
        &= \frac{\sum_{i\sim j} v_i^2 + v_j^2 - \left(v_i - v_j\right)^2}{2 \sum_{i=1}^{\lvert V(G)\rvert}v_i^2}\\
        &= \frac{\sum_{i\sim j} v_i^2}{\sum_{i=1}^{\lvert V(G)\rvert}v_i^2} - \frac{\sum_{i\sim j}\left(v_i - v_j\right)^2}{2 \sum_{i=1}^{\lvert V(G)\rvert}v_i^2}\\
        &= \frac{d\cdot \sum_{i=1}^{\lvert V(G)\rvert}v_i^2}{\sum_{i=1}^{\lvert V(G)\rvert}v_i^2} -  \frac{2\cdot \sum_{i\in S \land j \in \overline S}\left(v_i - v_j\right)^2}{2 \sum_{i=1}^{\lvert V(G)\rvert}v_i^2}\\
        &= d - \frac{\lvert \partial S \rvert \cdot \left(\frac{1}{\lvert S \rvert} + \frac{1}{\lvert \overline S \rvert}\right)^2}{\lvert S\rvert \frac{1}{\lvert S \rvert^2} + \lvert \overline S\rvert \frac{1}{\lvert \overline S \rvert^2}}\\
        &= d - \lvert \partial S \rvert\left(\frac{1}{\lvert S \rvert} + \frac{1}{\lvert \overline S \rvert}\right) \\ 
        &= d - \frac{ \lvert \partial S \rvert}{\lvert S \rvert}\frac{\left( \lvert S \rvert + \lvert \overline S \rvert\right)}{ \lvert \overline S \rvert}\\
        &= d- c(G) \frac{V(G)}{\lvert \overline S \rvert}\\
        &\geq d - 2c(G)
    \end{align*}
    Torej velja
    \begin{align*}
